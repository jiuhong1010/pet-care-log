...
业务插画统一从 `FeatureIllustration` 语义入口调用，分别覆盖观察变化、疫苗与驱虫、体重、用药接力、就诊和看诊包。资产位于 `src/assets/generated/feature-illustrations/`，使用独立透明 PNG、相同安全边距、居中视觉质量和统一底部基线；CSS 槽位一律 `object-fit: contain`，禁止用 `cover` 裁切。插画只用于主动作、业务入口、空状态和内容缩略图，不用于设置、箭头、关闭或选中状态。
...
当前宠物和 30 天照护状态先出现，但不再用整块深色抢走首屏。「今天」只保留主动作「记下刚刚的变化」、三步闭环解释、到期安排和当天用药接力。时间线与看诊包升为一级入口，分别承载连续观察和看诊前整理；长期档案集中疫苗、体重、用药和就诊，并用非对称插画入口建立辨识度。时间线用一条克制的弯曲路径连接节点，看诊包按「发生了什么、现在怎样、正在使用什么、想问什么」组织资料。
