...
当前宠物和下一件照护事项先出现，但不再用整块深色抢走首屏。「今天」采用插画与数据共同构成的照护轨迹：当前宠物的独立插画成为主视觉，切换宠物时用短暂淡入和位移完成同步切换；代码绘制的曲线实时读取最近观察、当前用药和最近到期事项，点击节点进入对应记录流程。看诊包紧接轨迹出现，主动作「记下刚刚的变化」位于视觉路径终点。时间线与看诊包是一级入口；长期档案集中疫苗、体重、用药和就诊。看诊包按「发生了什么、现在怎样、正在使用什么、想问什么」组织资料。
