...
宠物头像是身份内容而非界面 glyph，使用同一套生成式肖像图集。为保留耳朵、项圈、胸口、翅膀和龟壳等识别细节，统一采用约 5:6 的竖向圆角肖像，不再使用圆形大头裁剪。主状态区采用 90×108px（桌面 100×120px），切换器使用 28×34px 缩略图，选择器使用 74×88px 预览；外框尺寸保持一致，每种动物分别设置垂直焦点、视觉倍率和细微位移，并对跨越图集格线的相邻素材设置独立边界遮罩，避免串格残片。已有数据仍保存原 emoji 值，仅在显示层映射为肖像，保证旧记录和备份兼容。
